...
const bookList = [
  ...
  */
  ...
  const add_entry = (entry) => bookList.push(entry)

  // (2) Write an arrow function that removes a specific book from the bookList
  const remove_entry = (title) => {
    const index = bookList.map(book => book.title).indexOf(title)
    bookList.splice(index, 1)
  }
  // (3) Write an arrow function that lists out all the books or all the authors in the book list */
  const list_books = () => console.log(bookList.map(book => book.title));
  ...
  const pos_or_neg = (num) => num > 0 ? "positive" : "negative";
